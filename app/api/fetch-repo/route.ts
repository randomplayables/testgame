...
//     // NEW: Always provide a JS eslint config stub so TS/StackBlitz stop looking for a missing file
      ...
      if (item.type === "blob" && item.path && item.sha) {
        try {
          ...
          if (item.path === "src/services/apiService.ts") {
            ...
            // If the pattern didn't exist, inject a safe default near the top
            if (content === before) {
              content = `const API_BASE_URL = import.meta.env.VITE_API_BASE_URL || '/api/sandbox';\n` + content;
            }
          }
