  ...
  window.fetch = (input, init) => {
    ...
    if (typeof url === 'string' && (url.startsWith('/api/') || url.includes('/api/sandbox'))) {
      ...
      return new Promise((resolve, reject) => {
          ...
          if (ev.data && ev.data.type === 'RP_FETCH_RESULT' && ev.data.id === id) {
            ...
            try {
              ...
              if (isSessionCreate && ev.data.body) {
                ...
                if (parsed && parsed.sessionId) {
                  console.log('[Bridge] Announcing session ID:', parsed.sessionId);
                  // IMPORTANT: post to the TOP window so the host receives it.
                  window.top.postMessage({ type: 'SANDBOX_SESSION_ID', payload: parsed.sessionId }, '*');
                  // Make it available to the preview page too
                  window.__RP_SESSION_ID__ = parsed.sessionId;
                }
              }
            } catch (e) {
        ...
        console.log('[Bridge] Sending fetch request to parent:', { url, method });
        // IMPORTANT: send to TOP (not just the StackBlitz parent) so the host page sees it.
        window.top.postMessage({ type: 'RP_FETCH', id, input: { url, init } }, '*');
      });
    }
